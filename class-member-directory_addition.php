			...
			switch ( $this->filter_types[ $filter ] ) {
				...
				case 'select': {
					...
					if ( ! empty( $attrs['options'] ) || ! empty( $custom_dropdown ) ) { ?>							
							<?php
							// custom meta field do not have a label, so we have to set it manually.
							// hardcoded only for Career status
							if ($filter == 'current_career_status'){								
								$data_label = 'Career status';
							}
							else {
								$data_label = stripslashes( $attrs['label'] );
							} ?>
								
							<select class="um-s1" id="<?php echo $filter; ?>" name="<?php echo $filter; ?>"
						        data-placeholder="<?php esc_attr_e( $data_label, 'ultimate-member' ); ?>"
